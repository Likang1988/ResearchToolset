                    ...
                    for item in items:
                        if not item.parent_id:  # 顶级预算项
                            ...
                            for i in range(parent_item.childCount()):
                                if parent_item.child(i).text(0) == item.category.value:
        ...
        try:
            ...
            if item.parent():  # 不保存根节点
                ...
                if parent_text and parent_text in [cat.value for cat in BudgetEditCategory]:
                    budget_item.category = BudgetEditCategory(parent_text)
                item_text = item.text(0) if item.text(0) else None
                if item_text and item_text in [subcat.value for subcat in BudgetEditSubCategory]:
                    budget_item.subcategory = BudgetEditSubCategory(item_text)
        ...
        for category in BudgetEditCategory:
            ...
            for subcategory in BudgetEditSubCategory:
                ...
                if category == BudgetEditCategory.DIRECT and subcategory in [
                    ...
                    BudgetEditSubCategory.EXTERNAL,
                    BudgetEditSubCategory.FUEL,
                    BudgetEditSubCategory.TRAVEL,
                    BudgetEditSubCategory.PUBLICATION,
                    BudgetEditSubCategory.LABOR,
                    BudgetEditSubCategory.EXPERT,
                    BudgetEditSubCategory.OTHER
                ]:
